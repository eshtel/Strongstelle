...
// Check if a day has passed since the last save
function checkAndSaveCaloriesForDay() {
    const currentDateStr = currentDate.toLocaleDateString("fr-FR");

    if (lastSaveDate !== currentDateStr) {
        // Save the total calories for today to the stats page
        saveCaloriesForToday();

        // Update the lastSaveDate in localStorage
        localStorage.setItem("lastSaveDate", currentDateStr);
    }
}

// Function to save total calories to daily stats
...
// Call the function to check and save calories
checkAndSaveCaloriesForDay();
